...
void CShop::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			int ItemIndex,ItemLevel,ItemDurability,ItemOption1,ItemOption2,ItemOption3,ItemNewOption,ItemSet;
			...
			this->InsertItem(ItemIndex,ItemLevel,ItemDurability,ItemOption1,ItemOption2,ItemOption3,ItemNewOption,ItemSet);	
		}
	}
	catch(...)
	{
...
void CShop::InsertItem(int ItemIndex,int ItemLevel,int ItemDurability,int ItemOption1,int ItemOption2,int ItemOption3,int ItemNewOption,int ItemSet) // OK
{
	ITEM_INFO ItemInfo;
		...
		for(int x=0;x < 8;x++)
		{
			if(this->m_InventoryMap[((y*8)+x)] == 0xFF)
			{
				...
				if(slot != 0xFF)
				{
					...
					this->m_Item[slot].Convert(ItemIndex,ItemOption1,ItemOption2,ItemOption3,ItemNewOption,ItemSet);
