// Fruit.cpp: implementation of the CFruit class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "Fruit.h"
#include "DefaultClassInfo.h"
#include "Message.h"
#include "Notice.h"
#include "ObjectManager.h"
#include "ServerInfo.h"
#include "Util.h"

CFruit gFruit;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CFruit::CFruit() // OK
{
	this->Init();
}

CFruit::~CFruit() // OK
{

}

void CFruit::Init() // OK
{
	int NRMaxFruitPoint = 2;
	int MGMaxFruitPoint = 2;
	for(int n=0;n < MAX_CHARACTER_LEVEL;n++)
	{
		if(((n+1)%10) == 0)
		{
			NRMaxFruitPoint += (((n+11)*2)/400)+2;
			MGMaxFruitPoint += (((n+11)*2)/700)+2;
		}

		this->m_NRMaxFruitPointTable[n] = NRMaxFruitPoint;
		this->m_MGMaxFruitPointTable[n] = MGMaxFruitPoint;
	}
}

int CFruit::GetMaxFruitPoint(LPOBJ lpObj) // OK
{
	if(lpObj->Level < 1 || lpObj->Level > gServerInfo.m_MaxLevel)
	{
		return 0;
	}

	if(lpObj->Class == CLASS_MG)
	{
		return this->m_MGMaxFruitPointTable[lpObj->Level-1];
	}

	return this->m_NRMaxFruitPointTable[lpObj->Level-1];
}

bool CFruit::UseFruitItem(LPOBJ lpObj,int type) // OK
{
	PMSG_FRUIT_RESULT_SEND pMsg;

	pMsg.header.set(0x2C,sizeof(pMsg));

	if(lpObj->Level < 10 || type > 3)
	{
		pMsg.result = 0x40;
		DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
		return 0;
	}

	if(lpObj->FruitPoint >= this->GetMaxFruitPoint(lpObj))
	{
		pMsg.result = 0xC0;
		DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
		return 0;
	}

	int* stat;

	switch(type)
	{
		case 3:
			stat = &lpObj->Strength;
			break;
		case 2:
			stat = &lpObj->Dexterity;
			break;
		case 1:
			stat = &lpObj->Vitality;
			break;
		case 0:
			stat = &lpObj->Energy;
			break;
	}

	if((*stat) >= gServerInfo.m_MaxStatPoint[lpObj->AccountLevel])
	{
		pMsg.result = 0xC0;
		DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
		return 0;
	}

	int rate = 0;

	if(lpObj->FruitPoint <= 10)
	{
		rate = 100;
	}
	else if((lpObj->FruitPoint-10) < ((this->GetMaxFruitPoint(lpObj)*10)/100))
	{
		rate = 70;
	}
	else if((lpObj->FruitPoint-10) < ((this->GetMaxFruitPoint(lpObj)*30)/100))
	{
		rate = 60;
	}
	else if((lpObj->FruitPoint-10) < ((this->GetMaxFruitPoint(lpObj)*50)/100))
	{
		rate = 50;
	}
	else if((lpObj->FruitPoint-10) < ((this->GetMaxFruitPoint(lpObj)*80)/100))
	{
		rate = 40;
	}
	else
	{
		rate = 30;
	}

	if((GetLargeRand()%100) < ((gServerInfo.m_FruitAddPointSuccessRate[lpObj->AccountLevel]==-1)?rate:gServerInfo.m_FruitAddPointSuccessRate[lpObj->AccountLevel]))
	{
		int amount = gServerInfo.m_FruitAddPointMin+(GetLargeRand()%((gServerInfo.m_FruitAddPointMax-gServerInfo.m_FruitAddPointMin)+1));

		amount = (((lpObj->FruitPoint+amount)>this->GetMaxFruitPoint(lpObj))?(this->GetMaxFruitPoint(lpObj)-lpObj->FruitPoint):amount);

		amount = ((((*stat)+amount)>gServerInfo.m_MaxStatPoint[lpObj->AccountLevel])?(gServerInfo.m_MaxStatPoint[lpObj->AccountLevel]-(*stat)):amount);

		(*stat) += amount;

		lpObj->FruitPoint += amount;

		pMsg.result = amount;
		pMsg.result |= (type << 4);

		DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);

		gObjectManager.CharacterCalcAttribute(lpObj->Index);
	}
	else
	{
		pMsg.result = 0x40;

		DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
	}

	return 1;
}