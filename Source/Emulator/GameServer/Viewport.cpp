...
void CViewport::GCViewportMonsterSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		info.DirAndPkLevel = (lpTarget->Dir*16);
...
void CViewport::GCViewportGuildSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		info.index[0] = SET_NUMBERHB(lpTarget->Guild->Index);
		info.index[1] = SET_NUMBERLB(lpTarget->Guild->Index);

		memcpy(info.name,lpTarget->Guild->Name,sizeof(info.name));
	...
	this->GCViewportGuildMemberSend(aIndex);
}

void CViewport::GCViewportGuildMemberSend(int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		info.number[0] = SET_NUMBERHB(lpTarget->Guild->Index);
		info.number[1] = SET_NUMBERLB(lpTarget->Guild->Index);
...
void CViewport::GCViewportSimpleGuildSend(LPOBJ lpObj) // OK
{
	...
	info.index[0] = SET_NUMBERHB(lpObj->Guild->Index);
	info.index[1] = SET_NUMBERLB(lpObj->Guild->Index);

	memcpy(info.name,lpObj->Guild->Name,sizeof(info.name));
	...
	this->GCViewportSimpleGuildMemberSend(lpObj);
}

void CViewport::GCViewportSimpleGuildMemberSend(LPOBJ lpObj) // OK
{
	if(lpObj->Guild == 0)
	{
		return;
	...
	info.number[0] = SET_NUMBERHB(lpObj->Guild->Index);
	info.number[1] = SET_NUMBERLB(lpObj->Guild->Index);
